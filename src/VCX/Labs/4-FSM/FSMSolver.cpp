...
namespace VCX::Labs::FSM {
    FSMSolver::FSMSolver () {}
    FSMSolver::FSMSolver (MassSpringSystem const & system, float _dt, int _iters):
    dt(_dt), iters(_iters)
    {
        // Init L, J, M
    ...
    */
    void FSMSolver::Solve (MassSpringSystem const & system) {
        ...
        Eigen::VectorXf y = x + system.Damping * dt * v;
        Eigen::VectorXf b = dt*dt*J*d + M*y - dt*dt*f_ext;

        Eigen::VectorXf x_new = ComputeSimplicialLLT(A, b);
        Eigen::VectorXf v_new = (x_new - x) / dt;
